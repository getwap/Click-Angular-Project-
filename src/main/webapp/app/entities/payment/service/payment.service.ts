...
import {IPayment, getPaymentIdentifier, Payment} from '../payment.model';
...
export class PaymentService {
  protected resourceUrl2 = this.applicationConfigService.getEndpointFor('api/amount')
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/payments');

  constructor(protected http: HttpClient, protected applicationConfigService: ApplicationConfigService, sharedConfirmService: SharedconfirmService) {}

  amount(payment: IPayment): Observable<EntityResponseType>{
    return this.http.post<IPayment>(this.resourceUrl2, payment,{observe: 'response'});
  }
